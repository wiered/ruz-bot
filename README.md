...
- `REDIS_TTL_SCHEDULE_S` - TTL структурированного недельного расписания;
...
- недельное расписание как общий источник для `Сегодня`, `Завтра`, `Пред. день`, `След. день`, `Эта неделя` и `Следующая неделя`;
